...
- `project_designsystem.md` — Farb-Hintergrund-Matrix, Pastell-Bewertung, Smart-Icon, Tooltip-Pattern. **„Dunkel-auf-dunkel" ist No-Go**.
- `project_steigerungs_heuristik.md` — Web liefert Gefühl, Excel liefert Tiefe, Beratung liefert Begleitung
- `project_hybrid_risiko_modellierung.md` — 10 Sensibilisierungs-Punkte plus Investitionsplan-Belege
- `project_excel_patch_v2_1.md` — Excel-Folgeaufgaben (parallel zu v2.0, gebündelt)
- `feedback_excel_kontinuierliche_optimierung.md` — bidirektionale Optimierung
- `feedback_eigentuemer_perspektive.md` — Vermieter-Sicht-Patterns
...
5. **Sensibilisierungs-Block fossile Energie** als neue Standard-Komponente unter Wirtschaftlichkeit (statt Risiko-Übersicht/Sensitivität, die in die Excel verschoben werden). 10 Punkte in Kontextblau mit Smart-Icon.

6. **Wizard erweitert** auf 4 Schritte mit ~17 Eingaben (statt 9 Pflichtfelder + PV-Toggle). PV raus, dafür Energiepreis-Eingabe + aktuelle Jahreskosten + Sanierungs-Daten + Erhaltungsrücklage rein.
...
1. **Eine substantielle Eingabe-Maske** (4 Schritte, ~17 Felder), die das Gefühl vermittelt: hier wird ganzheitlich gerechnet
...
6. **Eine Sensibilisierungs-Erweiterung** mit 10 Punkten zu fossilen Risiken (sichtbar bei jeder Empfehlung)
...
- Wizard-Restrukturierung (4 Schritte, ~17 Felder, PV raus, Sanierungs-Daten + Energiepreise rein)
...
- Sensibilisierungs-Block fossile Energie (10 Punkte) als neue Standard-Komponente
...
2. **Wizard-Schritt 2 (Heizung & Verbrauch, 6 Felder):** Aktuelle Heizung + Baujahr, Wohnfläche, Heizlast (mit Smart-Default + ⓘ), Jahresverbrauch, Aktuelle Jahreskosten aus Abrechnung, Aktueller Energiepreis (ct/kWh, energieträger-spezifisch)
...
9. **Erläuterungs-Box rechts oben** an der Wirtschaftlichkeits-Tabelle (klappbar, Smart-Icon-Trigger), mit Lese-Hilfe und Plausibilitäts-Aussage zur Erhaltungsrücklage („deine Rücklage 6,61 €/m²/a liegt unter GdW-Richtwert 13–17 €/m²/a — bei Sanierungshorizont 10 J reicht das nicht")
...
11. **Sensibilisierungs-Block fossile Energie** wird unter Wirtschaftlichkeits-Panel sichtbar, in Kontextblau, mit den 10 Punkten aus Memory `project_hybrid_risiko_modellierung.md`
...
19. **Sondersituations-CTA** am Ende, generisch formuliert: „Du hast eine Sondersituation — Eigentümerversammlung, Sanierungsplanung, individuelle Heizungs-Frage? Schreib uns: dialog@hausentscheider.de. Wir lesen, wir antworten, wir begleiten dich."
...
## 3 — Wizard (4 Schritte, ~17 Eingaben)
...
**Schritt 2 — Heizung & Verbrauch (6 Felder):**
...
| 8 | Heizlast | Slider 5–500 kW + ⓘ-Tooltip „Was ist Heizlast? Smart-Default aus Wohnfläche × Sanierungsfaktor (DIN EN 12831-1)" | `HeizlastKW` | Wohnfläche × Sanierungsfaktor |
| 9 | Jahresverbrauch | Slider 5.000–500.000 kWh | `VerbrauchIst` | 95.000 (MFH) / 18.000 (EFH) |
| 10 | Aktuelle Jahreskosten aus Abrechnung | €-Eingabe | `JahreskostenIst` | leer (User trägt aus Abrechnung ein) |
| 11 | Aktueller Energiepreis | ct/kWh-Eingabe (energieträger-spezifisch je nach `HeizungAktuell`) | `EigenerPreisAktuell` | Markt-Schnitt aus `preishistorie.json` |
...
| # | Feld | Typ | Defined-Name | Default |
|---|---|---|---|---|
| 12 | Sanierungsstand | Toggle „unsaniert / teilweise saniert / saniert" + ⓘ-Erläuterung (ohne Jahreszahlen, dafür Erläuterung was unter „saniert" zählt) | `Sanierungsstand` | „teilsaniert" |
...
| 16 | Jährliche Sanierungs-Investition | €-Slider + ⓘ-Tooltip „Was möchtest du pro Jahr für Sanierung zurücklegen?" | `JaehrlSanierungsInvest` | 0 |
| 17 | Erhaltungsrücklage pro WE pro Monat | €-Eingabe + Live-Umrechnung auf €/m²/a unterhalb des Felds | `RuecklageProWEMonat` | 0 |

**Plausibilitäts-Hinweis Schritt 3 (live):**
- Live-Umrechnung Erhaltungsrücklage: User trägt €/WE/Monat ein → UI zeigt unter dem Feld dynamisch „= X €/m²/a (GdW-Richtwert 13–17 €/m²/a)"
...
(Hinweis: Schritt 4 hat 2 Felder, Wizard insgesamt 19 — leichte Korrektur zur „17"-Schätzung. Persona ist konzeptuell eigener Schritt, weil sie das Wertesystem prägt.)
...
(Keine Jahreszahlen mehr — Daniel-Befund 03.05.2026: Jahreszahlen irritieren, weil Theaterstraße 2010 saniert wurde, aber „2010" passt in keine der bisherigen Klassifikations-Klammern.)
...
├──────────────────────────────────────────────────────────────┤
│  7. SENSIBILISIERUNGS-BLOCK fossile Energie (10 Punkte)     │
...
**Pastell-Bewertung pro Zelle (siehe Memory `project_designsystem.md` §2):**
...
**Vermieter-Bilanz Block 4 (Risiko-Übersicht):** **RAUS** aus Web. In Excel verschoben (siehe `project_excel_patch_v2_1.md`).
...
- Listen-Padding korrigieren (mindestens 1.5 rem padding-left, siehe Daniel-Befund 03.05.: linkslastig)
- „muss" → „sollte" („was sollte beschlossen werden", nicht „was muss beschlossen werden")
...
**Inhalt (10 Punkte aus Memory `project_hybrid_risiko_modellierung.md`, finalisierte Version):**
...
11. Mieter-Vermieter-Aufteilungs-Trend — der Vermieter trägt
    ...
    Netzentgelte-Aufteilung (Bundesnetzagentur-Eckpunkte
    2024 zur „verursachergerechten Verteilung"). Trend: weitere
    Kosten-Komponenten (Grüngasquote-Aufschlag) werden ggf.
    künftig anteilig umgelegt. Wer in fossile Heizung investiert,
    kann nicht mehr darauf vertrauen, dass alle Folgekosten
    Mieter-Sache bleiben.
...
**Tonalität:** sachlich, ruhig, nicht alarmistisch (siehe Memory `project_designsystem.md` §6).

**Wo verlinken:** Punkt 9 zur Quelle science.lu (CO₂-Studie), Punkt 6 zu DVGW-Studie (H₂-Umrüst-Kosten), Punkt 10 Bezug auf Klima-Vertrag Deutschland 2045. Tooltip-Pattern für Quellen-Verlinkung.
...
(Die Werte werden algorithmisch in `engine.js` berechnet aus Wirtschafts-Werten, CO₂-Faktoren, Plausibilitäts-Filtern etc. — Heuristik oben dient zur Plausibilitäts-Prüfung.)
...
Das fertige Big-Picture-SVG wird als statisches Bild rechts neben „Zum Entscheider" auf der Startseite eingebunden — als visueller Anker dafür, was die Plattform liefert.
...
**Konzept:** Ein einziger aufklappbarer Block, der dem User einen kompakten Vorgeschmack auf die Excel-Tiefe gibt — ohne den Standard-Nutzer zu überfordern, aber mit Augenhöhe-Demonstration.
...
**Wichtig:** Kein neuer State, keine eigenen Schieberegler. Der Vorgeschmack-Aufklapper rendert mit dem aktuellen State der Wirtschaftlichkeits-Tabelle. Reagiert auf Schieberegler-Wirkung, aber bietet keine eigenen Hebel.

**Designsystem-Hinweis:** Aufklapp-Indikator (Pfeil/Plus) muss klar sichtbar sein. Bei aufgeklappt: Summary-Hintergrund leicht abgesetzt (Petrol-Light), bei zugeklappt: neutral.
...
Jede Kachel zeigt einen Excel-Tab als Bild plus 1–2 Sätze Beschreibung.

```
┌────────────────┐  ┌────────────────┐
│ [Excel-Bild]   │  │ [Excel-Bild]   │
│ TCO Detail     │  │ Cashflow 25 J  │
│                │  │                │
│ Komponenten,   │  │ Volle Jahres-  │
│ Annuität,      │  │ Tabelle pro    │
│ Methodik       │  │ Option         │
└────────────────┘  └────────────────┘

┌────────────────┐  ┌────────────────┐
│ [Excel-Bild]   │  │ [Excel-Bild]   │
│ Sanierungs-    │  │ Sensitivitäts- │
│ pfad           │  │ Analyse        │
│                │  │                │
│ Erhaltungs-    │  │ CO₂-Pfade,     │
│ rücklage,      │  │ Gas-Subvent.,  │
│ Investitions-  │  │ Stranded       │
│ Plan           │  │ Asset          │
└────────────────┘  └────────────────┘
```

**Bilder:** statische PNG/SVG-Screenshots aus der Excel, 4 Kacheln. Ablage: `assets/excel-vorschau/{tco|cashflow|sanierung|sensitivitaet}.png`. Erstellung als Folgeaufgabe (Daniel selbst oder Claude Code mit Screenshot-Skript).

### 11.3 Drei-Spalten-Tarif-Vergleich

```
┌─────────────────┬─────────────────┬─────────────────┐
│   WEB           │   EXCEL         │   JÄHRLICHES    │
│   KOSTENLOS     │   EDITION       │   UPDATE        │
│                 │   einmalig      │   pro Jahr      │
│   0 €           │   49 €          │   19 €          │
├─────────────────┼─────────────────┼─────────────────┤
│ ✓ Dashboard     │ ✓ Alle Web-     │ ✓ Aktualisierte │
│   Vergleich     │   Inhalte       │   Marktpreise   │
│                 │                 │                 │
│ ✓ Sensibili-    │ ✓ TCO Detail    │ ✓ Neue Quellen  │
│   sierung       │   Annuität      │                 │
│                 │                 │                 │
│ ✓ Big Picture   │ ✓ Volle Cash-   │ ✓ Methodik-     │
│   Visualisier.  │   flow-Tabelle  │   Verfeinerung  │
│                 │                 │                 │
│ ✓ Schieberegler │ ✓ Sanierungs-   │ ✓ Neue          │
│   Was-wäre-wenn │   pfad-Tab      │   Sensitivitäts-│
│                 │                 │   Szenarien     │
│                 │                 │                 │
│                 │ ✓ Sensitivität  │ ✓ E-Mail        │
│                 │   frei wählbar  │   Direktversand │
│                 │                 │                 │
│ ✗ Volle TCO-    │ ✓ §7b AfA-      │                 │
│   Tabellen      │   Detail        │                 │
│                 │                 │                 │
│ ✗ Sensitivität  │ ✓ Mietspiegel-  │                 │
│   frei wählbar  │   Klassen-      │                 │
│                 │   Lookup        │                 │
│                 │                 │                 │
│ ✗ §7b AfA       │ ✓ KfW-Kombi-    │                 │
│   Detail        │   Modell        │                 │
...
│                 │   wärme-Logik   │                 │
│                 │                 │                 │
│                 │ [Jetzt kaufen]  │ [Update bestell.│
└─────────────────┴─────────────────┴─────────────────┘
```

**Wichtig:**
- 3 gleichgroße Spalten, klare Vor- und Gegenüberstellung
- Web-Spalte zeigt auch Was-NICHT-drin-ist (✗ = klar, was Excel liefert was Web nicht)
- Excel-Spalte zeigt was alles drin ist + Kauf-Button
- Update-Spalte zeigt was jährliche 19 € liefern + Update-bestell-Button
- Layout responsive — auf Mobile vertikal gestapelt

**Tonalität:** kein Verkaufs-Stil, sondern Werkzeug-Übersicht. „Hier ist die Erweiterung — du entscheidest" statt „Kauf jetzt!".
...
**Pattern:** horizontales Scroll-Regal (siehe Memory `project_designsystem.md` §5).

**Initiale Karten (5+):**

```
┌─────────────┐ ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ ┌─────────────┐
│ BAFA        │ │ KfW         │ │ Solarrechn. │ │ Verbrauch.- │ │ SWE         │
│ Förderung   │ │ Kredit      │ │ Thüringen   │ │ Zentrale    │ │ Erfurt      │
│ beantragen  │ │ prüfen      │ │             │ │ Beratung    │ │             │
│ → bafa.de   │ │ → kfw.de    │ │ → tega.de   │ │ → vz...     │ │ → swe...    │
└─────────────┘ └─────────────┘ └─────────────┘ └─────────────┘ └─────────────┘
```

**Erweiterbar:** wenn künftig weitere Karten dazukommen (z. B. „Energieberater finden", „Heizungsbauer in deiner Region", „WEG-Verwalter"), passt das Layout ohne Bruch.
...
**Wichtig:**
- Generisch formuliert, nicht spezifisch („Eigentümerversammlung" als ein Beispiel von vielen)
- KEIN Beratungs-CTA mehr in den WEG-Hinweisen oben — alles am Ende gebündelt
- KEIN „Link teilen"-Button (Daniel-Befund 03.05.: war doppelt mit Analyse-Senden)
...
**Vollständig in Memory `project_designsystem.md` dokumentiert.** Hier nur Verweis und drei kritische Punkte:

1. **„Dunkle Schrift auf dunklem Hintergrund" ist No-Go.** Audit beim Browser-Test mit DevTools. Insbesondere prüfen: Methodik-Tooltips (Annahmen-Box-Header) und alle `<details>`-Inhalte.

2. **Pastell-Bewertung in allen Vergleichs-Tabellen.** Algorithmus zentral in `engine.js`, Render in `ergebnis.js`. Pellets bei `pelletsPlausibel = false` ausgegraut.

3. **Smart-Icon (Glühbirne mit Strahlen-Aura)** als Marken-Element bei Risiko-Banner, Sensibilisierungs-Block, Methodik-Tooltips für Antizipations-Inhalte. SVG-Erstellung als Folgeaufgabe — bis dahin Platzhalter.
...
├── daten/
│   ├── parameter.json               (existiert, ggf. Erweiterung wenn build_json.py-Folgeaufgaben erledigt)
│   ├── preishistorie.json           (existiert)
│   └── glossar.json                 (NEU — aus Excel-Tab Begriffe via build_json.py)
├── assets/
│   ├── icons/
│   │   └── hausentscheider-glühbirne.svg   (NEU — Marken-Smart-Icon, eigene Erstellung)
│   ├── excel-vorschau/
│   │   ├── tco-detail.png           (NEU — Excel-Screenshot, Daniel oder Skript)
│   │   ├── cashflow-25j.png         (NEU)
│   │   ├── sanierungspfad.png       (NEU)
│   │   └── sensitivitaet.png        (NEU)
│   └── big-picture-referenz.svg     (NEU — Big-Picture-Standbild für Startseite)
├── docs/
│   ├── C1_BlockSpec_v3.md
│   ├── C2_BlockSpec_v1.md           (alt v1, kann durch v2.0 überschrieben werden)
│   └── C2_BlockSpec_v2.0.md         (NEU — Snapshot dieser Spec)
├── README.md                        (existiert, ggf. erweitern um lokale Test-Anleitung)
├── build_json.py                    (existiert, Erweiterung für glossar.json + Folgeaufgaben)
└── …
```
...
- daten/glossar.json (NEU — nicht da, kann erstmal als JS-Konstante in
...
2. rechner.html — Ergebnis-Seite-Reihenfolge nach Spec §4:
   - Annahmen-Box / Headline-Antwort / Risiko-Banner / Persona-Toggle /
     Mini-Cockpit / Zeitraum-Schieberegler (NEU OBEN)
   - Wirtschaftlichkeits-Panel als Excel-Dashboard-Tabelle (NEU §5)
   - Sensibilisierungs-Block fossile Energie (NEU §7)
   - Vermieter-Bilanz Block 1+2+3 (mit Pastell)
   - WEG-Hinweise (ohne doppelten CTA)
   - Panel 2/3 Stubs
   - Was-wäre-wenn (KEIN Zeitraum mehr, nur Energiepreise + Förderung)
   - Big Picture (NEU §9, mit 6 Achsen)
   - Vorgeschmack-Aufklapper (NEU §10)
   - Excel-Edition mit 4 Referenzbild-Kacheln + Tarif-Vergleich (NEU §11)
   - Wegweiser horizontales Scroll-Regal (NEU §12)
   - Sondersituations-CTA generisch am Ende (NEU §13)

3. js/engine.js — erweitert um:
   - berechneAllDashboardKennzahlen(state, params) für Wirtschaftlichkeits-
     Tabelle (alle Zeilen aus Excel-Tab Dashboard)
   - bewerteZelle(wert, alleWerteZeile, optionPlausibel) für Pastell-Logik
   - berechneBigPictureAchsen(state, params) für 6-Achsen-Werte pro
     Heizoption (heuristisch laut Spec §9, Diskussion v2.1)
   - berechneErhaltungsruecklageStatus(state) für Plausibilitäts-Hinweis
     (€/m²/a vs. GdW-Richtwert)

4. js/ergebnis.js — erweitert um:
   - renderWirtschaftlichkeitsTabelle (NEU)
   - renderSensibilisierungsBlock (NEU)
   - renderBigPicture (NEU, Chart.js Radar-Chart, 5 Datasets)
   - renderVorgeschmackAufklapper (NEU, kompakt mit TCO-Komponenten +
     Cashflow-Kurve aus v1.1)
   - renderExcelEditionSektion (NEU, mit Referenzbild-Kacheln + Tarif-
     Vergleich)
   - renderWegweiserScrollRegal (NEU)
   - renderSondersituationsCTA (NEU, generisch am Ende)
   - renderTooltipFromGlossar (NEU, einheitliches Tooltip-Pattern)
   Plus: vermieterBilanz-Tabellen (Block 2+3) auf Pastell-Bewertung
   umstellen.

5. js/glossar.js — NEU, Lookup-Konstante für Abkürzungen und Begriffe
   (EPBD, BAFA, KfW, GMG, FRI, BEG, BEHG, EU-ETS, MEA, JAZ, DVGW, AGFW,
   Annuität, Barwertfaktor, Sanierungsstand, Sondereigentum, Vermögens-
   bilanz). Format wie Spec §14 Memory-Verweis. Folgeaufgabe build_json.py
   für Excel-Tab-Begriffe-Sync.

6. Verbindliche Memory-Regeln beachten (Pflicht):
   - project_designsystem.md §1 — Farb-Hintergrund-Matrix, KEIN
     Dunkel-auf-dunkel
   - project_designsystem.md §2 — Pastell-Bewertung in allen Tabellen
   - project_designsystem.md §3 — Smart-Icon (Glühbirne mit Strahlen-Aura,
     Filament als Hausdach) bis SVG erstellt: Platzhalter „💡✦"
     verwenden
   - project_designsystem.md §4 — Tooltip-Pattern einheitlich
   - project_designsystem.md §5 — Scroll-Regal bei > 4 Karten
   - project_designsystem.md §6 — Tonalität (Du-Form, kein Marketing-
     Buzz, „sollte" statt „muss")
   - project_designsystem.md §7 — Listen-Padding ≥ 1.5rem, <details> open
     bei dynamisch eingeblendeten Inhalten, Whitespace 1.5–2rem

7. docs/C2_BlockSpec_v2.0.md — Snapshot der Spec ins Repo (überschreibt
   alte v1).

Akzeptanz-Kriterien siehe Spec §2 — alle 30 Punkte. Insbesondere:
- Wizard 4 Schritte funktional, 19 Felder mit konditionalen
  Verzweigungen
- Wirtschaftlichkeits-Tabelle mit Pastell-Bewertung auf Mobile als
  Karten-Stack
- Sensibilisierungs-Block mit 10 Punkten in Kontextblau
- Big Picture mit 6 Achsen, dynamisch reagierend auf Schieberegler
- Vorgeschmack-Aufklapper kompakt
- Excel-Edition mit 4 Kacheln (zunächst Platzhalter-Bilder) + Tarif-
  Vergleich
- Wegweiser-Scroll-Regal funktional auf Touch
- Sondersituations-CTA einmal am Ende, generisch
- Designsystem-Konformität geprüft (DevTools-Audit, kein Dunkel-auf-
  dunkel)
...
- Berechnungs-Funktionen aus engine.js UNVERÄNDERT — nur erweitern,
  nicht umschreiben
- Pellets-Filter: bleibt wie v1.1 (Innenstadt + > 30 kW oder > 6 WE
  oder MFH-Innenstadt)
...
Erwartete UX-Schliff-Schleifen (analog v1.1): 2–3 Schleifen mit Daniel
nach Initial-Implementation. Größerer Umbau als v1.1, daher Aufwand
6–8 h.
```
...
2. Claude Code legt Plan vor — strukturelle Restrukturierung, neue Render-Funktionen, Memory-Regeln-Berücksichtigung.
...
5. Daniel testet im Browser via `http://localhost:3000/rechner.html`. Akzeptanz-Kriterien-Audit.
6. UX-Schliff-Schleifen 1–3 wie bei v1.1.
7. Commit + Push durch Daniel über GitHub Desktop. Live-Check auf hausentscheider.de.
8. Tagesabschluss.

**Geschätzter Aufwand Daniel:** 6–8 h inkl. Plan-Prüfung, Implementations-Iterationen, Browser-Tests, DevTools-Audit, Mobile-Check, Commit.
...
|---|---|---|
| v1.0 | 03.05.2026 | Initial — folgt auf C1 v3, deckt Wirtschaftlichkeits-Panel + Vermieter-Bilanz + Erweiterter Modus + Methodik-Tooltips ab. |
| v1.1 | 03.05.2026 (Spätnachmittag) | Patch nach Excel-Lektüre Tab Rechenlogik durch Daniel. §3.1 TCO-Methodik komplett neu auf VDI 2067 Annuität, Reparaturen aus TCO entfernt, CO₂ in TCO ergänzt, Verifikations-Werte mit echten Excel-Werten ersetzt, §3.8 Sonderumlage + IRR + Modernisierungs-Anteil neu. |
| v2.0 | 04.05.2026 (heute) | **Konzeptionelle Neuausrichtung** nach zwei Feedback-Schleifen mit Daniel am 03.05.2026 abend. Sieben strukturelle Verschiebungen: Wirtschaftlichkeits-Panel = Excel-Dashboard-Tabelle mit Pastell, Zeitraum-Schieberegler oben, TCO-Detail in Vorgeschmack-Aufklapper, Big Picture mit 6 Achsen als Highlight, Sensibilisierungs-Block fossile Energie (10 Punkte), Wizard 4 Schritte ~19 Felder, Excel-Edition mit 4 Referenzbild-Kacheln + Tarif-Vergleich. Plus Wegweiser-Scroll-Regal, Sondersituations-CTA einmal generisch, Smart-Icon Glühbirne als Marken-Element. Verbindliches Designsystem-Memory. Bidirektionale Excel-Optimierung: Excel-Patch v2.1 als parallele Folge-Bauphase. |

---

**Spec-Ende.** Pfad: `00_Projektsteuerung/260504_C2_BlockSpec_v2.0.md`. Wird beim Commit der Implementation als Snapshot in `docs/C2_BlockSpec_v2.0.md` ins Repo gespiegelt (überschreibt alte v1).
